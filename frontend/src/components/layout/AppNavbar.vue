  ...
  <nav class="lm-navbar" :class="{ scrolled: isScrolled }">
    <div class="container-fluid px-4 h-100 d-flex align-items-center justify-content-between">
      ...
      <ul class="d-none d-lg-flex list-unstyled mb-0 gap-4 align-items-center">
        <li><RouterLink class="lm-nav-link" to="/">Trang Chu</RouterLink></li>
        <li><RouterLink class="lm-nav-link" to="/collections">San Pham</RouterLink></li>
        <li><RouterLink class="lm-nav-link" to="/wishlist">Yeu Thich</RouterLink></li>
        <li><RouterLink class="lm-nav-link" to="/profile">Tai Khoan</RouterLink></li>
      </ul>

      <div class="d-flex align-items-center gap-1">
        <button class="lm-nav-icon-btn" @click="toggleSearch" title="Tim kiem">
          <i class="bi bi-search"></i>
        </button>
        <button class="lm-nav-icon-btn" @click="$router.push('/wishlist')" title="Yeu thich">
          <i class="bi bi-heart"></i>
        </button>
        <button class="lm-nav-icon-btn" @click="openCart()" title="Gio hang">
          <i class="bi bi-bag"></i>
          <span class="lm-cart-badge" :style="badgeScale">{{ totalCount }}</span>
        </button>
        <button class="lm-nav-icon-btn d-none d-md-flex" @click="$router.push('/profile')" title="Tai khoan">
