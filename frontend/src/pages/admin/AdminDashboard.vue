<template>
  <AdminLayout>
    <h1
      class="z-display mb-1"
      style="font-size: 26px; font-weight: 500; color: var(--z-dark)"
    >
      Tổng quan
    </h1>
    <p style="font-size: 14px; color: var(--z-gray); margin-bottom: 24px">
      Chào mừng trở lại, Admin. Đây là tình hình cửa hàng hôm nay.
    </p>
    ...
    <div class="row g-3 mb-4">
      <div v-for="stat in stats" :key="stat.label" class="col-6 col-xl-3">
        <div class="z-stat-card">
          ...
          <div
            class="z-stat-change"
            :style="{ color: stat.up ? '#16a34a' : 'var(--z-accent)' }"
          >
            <i
              class="bi"
              :class="stat.up ? 'bi-arrow-up-right' : 'bi-arrow-down-right'"
            ></i>
          ...
          <table class="z-table">
            ...
            <tbody>
              <tr v-for="order in recentOrders" :key="order.id">
                ...
                <td>
                  <span class="z-status" :class="order.statusClass">{{
                    order.status
                  }}</span>
                </td>
        ...
        <div class="z-admin-card">
          ...
          <div class="d-flex flex-column gap-3">
            ...
            >
              ...
              <div
                style="font-size: 13px; font-weight: 600; color: var(--z-dark)"
              >
                {{ p.revenue }}
              </div>
            </div>
...
const toApiDateTime = (value, seconds) => `${value}:${seconds}`;

const getDefaultThongKeRange = () => {
  const start = new Date("2024-01-01T00:00:00");
  const end = new Date();
  end.setHours(23, 59, 59, 999);

  return {
    startDate: toApiDateTime(toLocalDatetimeInput(start), "00"),
    endDate: toApiDateTime(toLocalDatetimeInput(end), "59"),
  };
};
...
onMounted(async () => {
  try {
    const [s, orders, prods] = await Promise.all([
      ...
      api().getHoaDon(),
      api().getThongKeTongHop({
        ...getDefaultThongKeRange(),
        period: "month",
      }),
    ]);
    stats.value = [
      {
        label: "Doanh thu",
        value: fmtPrice(s.doanhThu),
        change: s.tongLoaiVay + " loại váy",
        up: true,
        icon: "bi-graph-up",
        color: "#16a34a",
      },
      ...
      {
        label: "Sản phẩm",
        value: String(s.tongSanPham),
        change: s.tongLoaiVay + " loại",
        up: true,
        icon: "bi-bag",
        color: "var(--z-warm)",
      },
    ...
    const bestSellers = Array.isArray(prods?.topSanPham)
      ? prods.topSanPham
      : [];
    topProducts.value = bestSellers.slice(0, 5).map((p, i) => ({
      name: p.ten || "N/A",
      sold: String(p.tongSoLuong ?? 0),
      revenue: fmtPrice(p.doanhThu),
      letter: (p.ten || "Z").charAt(0).toUpperCase(),
      bg: [
